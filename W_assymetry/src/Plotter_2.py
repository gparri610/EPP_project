...
def setStyleStack(hs, options = ""):
    ...
    hs.GetHistogram().GetXaxis().SetBinLabel(2,"M_W (GeV)")
...
def plotVar(var, samples,isData = False, logScale = False ):
    ...
    if var == "M_W_p":
        hs.GetXaxis().SetTitle("M_{#it{W}^{+}} (GeV)")
        hs.SetTitle("Mass distribution of the #it{W}^{+}-boson")
        
    elif var == ("W_p_MET"):
        hs.GetXaxis().SetTitle("#it{E}_{T} (GeV)")
        hs.SetTitle("MET distribution of #it{W}^{+}#rightarrow#mu^{+} #nu")
    elif var == ( "W_m_MET"):
        hs.GetXaxis().SetTitle("#it{E}_{T} (GeV)")
        hs.SetTitle("MET distribution of #it{W}^{-}#rightarrow#mu^{-} #bar{#nu}")
    elif var == "M_W_m":    
        hs.GetXaxis().SetTitle("M_{#it{W}^{-}} (GeV)")
        hs.SetTitle("Mass distribution of the #it{W}^{-}-boson")
